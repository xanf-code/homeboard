...
export default function MainKavach() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["kavachdata"],
    queryFn: async () => {
      const res = await fetch(
        "https://1a6c-49-205-128-93.ngrok-free.app/kavach/v1/data"
      );
      console.log(res);
  ...
  if (isLoading) {
    return <p>Loading Kavach Information...</p>;
  }

  if (error) {
    return <p>Error fetching Kavach data: {error.message}</p>;
  }

  return (
    <div className="flex-1 p-3 space-y-2">
      <h1 className="text-2xl font-bold">Kavach General Monitor 🇮🇳</h1>
      <p className="mb-4 text-sm text-gray-400">
        Last updated {getTimeAgo(data.metadata.timestamp)}
      </p>
      {data.status == "Kavach Up" && (
        <>
            ...
            <div>
              <div className="flex items-center mb-4">
                ...
                <p className="text-white font-bold text-lg">
                  {data.pi_information.name}
                </p>
              </div>
              <p className="text-gray-400">{data.pi_information.purpose}</p>
              <div className="flex items-center mt-2">
                <span className="text-gray-400 text-sm mr-2">Version:</span>
                <p className="text-gray-400 text-sm">
                  {data.pi_information.version}
                </p>
              </div>
            </div>
            <div className="mr-2">
                  ...
                  <div className="flex items-center">
                    <div
                      className={`w-4 h-4 rounded-full mr-2 ${
                        data.metadata.hinata_health.FTL === "UP"
